# (S) to 4 end_point (D1, D2, D3, and D4) through 2 distribution centers (N1 and N2).
# User input
import itertools
# suppliers = {'S'}
# good_types = {'A'}    #{'A', 'B'}
# end_point = {'D1', 'D2'}       #{'D1', 'D2', 'D3', 'D4'}
# middle_points = {'N1'}     #{'N1', 'N2'}

suppliers = {'S'}
good_types = {'A', 'B'}
end_point = {'D1', 'D2', 'D3', 'D4'}
middle_points = {'N1', 'N2'}

variables1 = []
for type in good_types:
    for sup in suppliers:
        for center in middle_points:
            variables1.append(f'{type}_{sup}_{center}')
            for hospital in end_point:
                variables1.append(f'{type}_{center}_{hospital}')

supp_to_dis = list('_'.join(chars) for chars in itertools.product(good_types, suppliers, middle_points))
dis_to_hos = list('_'.join(chars) for chars in itertools.product(good_types, middle_points, end_point))
# Add to Variables
all_paths = set(supp_to_dis + dis_to_hos)

# print(supp_to_dis)
# print(f"Number of paths for each type from suppliers to Distribution centers: {len(supp_to_dis)}")
# print(dis_to_hos)
# print(f"Number of paths for each type from Distribution Centers to end_point: {len(dis_to_hos)}")
...
for vacc_type in good_types:
    ...
    supply_value = [[f'{vacc_type}_S_{center}' for center in middle_points], [1, 1]]
...
for hospital in end_point:
    for vacc_type in good_types:
        ...
        demand_value = [[f'{vacc_type}_{center}_{hospital}' for center in middle_points], [1, 1]]
...
if __name__ == '__main__':
    ...
    print(f"Distribution Centers: {middle_points}")
